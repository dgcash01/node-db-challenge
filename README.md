...
- [x] Explain the difference between `Relational Databases` and `SQL`. -- RDBMS is the system, and SQL is the language used to interact with the system

- [x] Why do tables need a `primary key`? Data Normalization. -- Each database table needs a primary key because it ensures row-level accessibility

- [x] What is the name given to a table column that references the primary key on another table. -- Foreign Key.

- [x] What do we need in order to have a _many to many_ relationship between two tables. -- An intermediary table that references both of the other tables.
